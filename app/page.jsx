...
export default function Home() {
  ...
  return <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" style={{marginBottom:'5rem'}}>
            {
                posts.map((post)=>(
                    <Link key={post._id} href={'/post/'+ post._id}>
                        <div className="border border-gray-200 p-4">
                            ...
                            <p className="text-gray-600 pb-2">{post.short_description}</p>
                        </div>
                    </Link>
                ))
            }
            {!posts.length > 0 && inputRef.current.value && <p>No post avaivable for this query: <b>{inputRef.current.value}</b></p>    } 
        </div>
        </>
}
